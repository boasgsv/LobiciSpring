...
public class LivrariaMvcApplication {
	...
	public CommandLineRunner demo(IUsuarioDAO usuarioDAO, IClienteDAO clienteDAO, ILocadoraDAO locadoraDAO,
		...
		return (args) -> {
			...
			lc1.setCliente(c1);
			lc1.setLocadora(l1);
			lc1.setData(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(now.getTime()));
			locacaoDAO.save(lc1);
			...
			lc2.setCliente(c2);
			lc2.setLocadora(l2);
			lc2.setData(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(now.getTime()));
			locacaoDAO.save(lc2);
		};
	}
}
